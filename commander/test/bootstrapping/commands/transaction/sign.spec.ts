...
describe('transaction:sign command', () => {
	...
	describe('offline', () => {
		...
		describe('sign multi signature registration transaction', () => {
			const messageForRegistration = {
				...
				mandatoryKeys: [accountsForMultisignature.mandatoryOne.publicKey, accountsForMultisignature.mandatoryTwo.publicKey].sort((k1, k2) => k1.compare(k2)),
				optionalKeys: [accountsForMultisignature.optionalOne.publicKey, accountsForMultisignature.optionalTwo.publicKey].sort((k1, k2) => k1.compare(k2)),
			};
			...
			const sign1 = ed.signData(MESSAGE_TAG_MULTISIG_REG, networkIdentifier, messageBytes, accountsForMultisignature.mandatoryTwo.privateKey);
			decodedParams.signatures.push(sign1);
			console.log('sign1: ', sign1.toString('hex'));

			const sign2 = ed.signData(MESSAGE_TAG_MULTISIG_REG, networkIdentifier, messageBytes, accountsForMultisignature.mandatoryOne.privateKey);
			decodedParams.signatures.push(sign2);
			console.log('sign2: ', sign2.toString('hex'));

			const sign3 = ed.signData(MESSAGE_TAG_MULTISIG_REG, networkIdentifier, messageBytes, accountsForMultisignature.optionalOne.privateKey);
			decodedParams.signatures.push(sign3);
			console.log('sign3: ', sign3.toString('hex'));

			const sign4 = ed.signData(MESSAGE_TAG_MULTISIG_REG, networkIdentifier, messageBytes, accountsForMultisignature.optionalTwo.privateKey);
			...
			const unsignedMultiSigTransaction = codec.encode(transactionSchema, tx);
			const TAG_TRANSACTION = 'LSK_TX_';
			const decodedBaseTransaction: any = codec.decode(
				transactionSchema,
				unsignedMultiSigTransaction,
			);
			const signatureSender = ed.signDataWithPrivateKey(TAG_TRANSACTION, networkIdentifier, unsignedMultiSigTransaction, accountsForMultisignature.targetAccount.privateKey)
    		const signedTransaction = codec.encode(transactionSchema, { ...decodedBaseTransaction, signatures: [signatureSender]});
	...
	describe('online', () => {
		...
		describe('sign multi signature registration transaction', () => {
			...
			it.only('should return signed transaction for sender account', async () => {
				await SignCommandExtended.run(
					[
						unsignedMultiSigTransaction.toString('hex'),
						`--passphrase=${accountsForMultisignature.targetAccount.passphrase}`,
